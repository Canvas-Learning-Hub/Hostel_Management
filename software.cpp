...
// Constants
const int MAX_ROOMS =
    1000; // Adjusted to a larger number to accommodate all rooms
...
void initializeRooms() {
  ...
  // Initialize rooms for each village
  for (int v = 0; v < 6; ++v) {
    ...
    for (int b = 0; b < numBlocks; ++b) {
      ...
      for (int f = 0; f <= numFloors; ++f) { // Including ground floor
        for (int u = 1; u <= 4; ++u) {       // 4 units per block
